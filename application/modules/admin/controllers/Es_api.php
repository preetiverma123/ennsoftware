...
class Es_api extends API_Controller 
{

	function __construct()
	{
		parent::__construct();
		$this->load->model(array('enquiry_status_model'));
	}
	...
	public function tasks_get($id=false)
	{
		if(! $id)
		{
			$tasks = $this->enquiry_status_model->get_all(); // return all record
		} else {
			$tasks = $this->enquiry_status_model->get_task($id); // return a record based on id
		}
	...
	{	
		...
		if($this->post('id')){
			$this->enquiry_status_model->update_task($this->post('id'), $data);
		}else{
			$this->db->insert('enquires_status',$data);
	...
	public function tasks_delete($id=NULL)
	{
		if($id == NULL)
		{
			...
			$this->enquiry_status_model->delete_task($id);
