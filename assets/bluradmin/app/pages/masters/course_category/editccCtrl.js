(function () {
  ...
  angular.module('BlurAdmin.pages.masters')
      .controller('editccCtrl', editccCtrl);
	
  function editccCtrl($scope,$http,$window,$stateParams,toastr) {
	//alert(1211);
	
	//  alert($stateParams.id);
	 $http.get(SITE_URL+'/admin/cc_api/tasks/'+ $stateParams.id).success(function(data){
        $scope.tasks = data;
  ...
  $scope.addTask = function(){
    ...
    $scope.updateTask = function(task){
        ...
        $http.post(SITE_URL+'/admin/cc_api/tasks', updateTask).success(function(data){
        	$window.location.href = SITE_URL+'/admin/index#/masters/course_category';
